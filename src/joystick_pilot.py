...
prompt = gui.DlgFromDict(
    dictionary =exp_info, 
    title      ="Beta Burst Adaptation Task"
)
...
subj_dir      = os.path.join("/Users/jacopobarone/Desktop/VS_code/psychopy/data/adaptation_task/{}".format(subj_ID))
...
# target_location = rnd.choices(angle_list, k=Ntrial)
...
for trial in np.arange(Ntrial):
    ...
    if trial < 80: #no perturbation phase
        perturbation    = 0.0
    elif  80 <= trial <= 230: #adaptation phase
        perturbation    = radians(60)  
    else: #after-effect phase
        perturbation    = 0.0
    ...
    data_dict["cursor_displacement"].append(perturbation)
